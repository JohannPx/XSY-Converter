...
namespace XsyConverter;

static class Program
{
    const string AppName = "XSY Converter";
    const string ExeName = "XsyConverter.exe";
    const string GitHubApiUrl = "https://api.github.com/repos/JohannPx/XSY-Converter/releases/latest";
    const string ResourceName = "XsyConverter.XSY-Converter_latest.ps1";
    ...
    static async Task UpdateSilently()
    {
        ...
        http.DefaultRequestHeaders.Add("User-Agent", "XsyConverter");
        ...
        var tempExe = Path.Combine(Path.GetTempPath(), "XsyConverter_update.exe");
        ...
        var batchPath = Path.Combine(Path.GetTempPath(), "xsy_converter_update.cmd");
    ...
    {
        var scriptPath = Path.Combine(Path.GetTempPath(), $"XSY-Converter_{Guid.NewGuid():N}.ps1");
